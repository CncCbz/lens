...
async def test_gateway_key_spend_accumulates_from_terminal_logs(tmp_path) -> None:
    ...
    try:
        ...
        await _create_terminal_log(
            request_log_store, key_id=key.id, total_cost_usd=1.25
        )
        await _create_terminal_log(
            request_log_store, key_id=key.id, total_cost_usd=2.75
        )
    ...
    try:
        ...
        log = await _create_terminal_log(
            ...
            request_content=json.dumps(
                {"accessToken": "gateway-secret", "model": "model"}
            ),
            request_headers=json.dumps(
                {"Authorization": "Bearer gateway-secret", "x-request-id": "id"}
            ),
        )
        await _finish_log(
            request_log_store,
            ...
            response_content=json.dumps(
                {"refresh_token": "provider-secret", "result": "ok"}
            ),
            upstream_headers=json.dumps(
                {"x-api-key": "provider-secret", "content-type": "application/json"}
            ),
        )

        detail = await request_log_store.get_request_log(log.id)
        assert json.loads(detail.request_content or "{}") == {
            "accessToken": "<redacted>",
            "model": "model",
        }
        assert json.loads(detail.request_headers or "{}") == {
            "Authorization": "<redacted>",
            "x-request-id": "id",
        }
        assert json.loads(detail.upstream_headers or "{}") == {
            "x-api-key": "<redacted>",
            "content-type": "application/json",
        }
        assert json.loads(detail.response_content or "{}") == {
