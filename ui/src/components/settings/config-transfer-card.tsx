...
function parseStatsPreview(value: unknown): ConfigBackupStatsSnapshot {
  ...
  return {
    ...
    channel_daily: Array.isArray(value.channel_daily)
      ? value.channel_daily
      : [],
    dimension_daily: Array.isArray(value.dimension_daily)
      ? value.dimension_daily
      : [],
  };
}
        ...
        <CardContent className="flex flex-col gap-4 px-4 py-4 sm:px-5 sm:py-5">
          ...
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {titleForLocale(
                locale,
                "导入会替换现有渠道、模型组、设置、模型价格、定时任务和统计数据；如果备份包包含网关 API Key，也会一并覆盖。",
                "Import replaces existing channels, model groups, settings, model prices, cron jobs, and stats. If the backup contains gateway API keys, those keys are replaced as well.",
              )}
            </AlertDescription>
          </Alert>
