...
interface GatewaySettingsProps {
  ...
  onGlobalHeadersChange: (value: string) => void;
  onAddRule: () => void;
  onUpdateRule: (
    index: number,
      ...
      <div className="flex flex-col gap-5 rounded-lg border bg-muted/20 p-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            {titleForLocale(
              locale,
              "作用于全部渠道的默认头。渠道组合里的请求头优先。",
              "Defaults for all channels. Combination headers win.",
            )}
          </p>
        </div>
        <Field>
          <FieldLabel>
            {titleForLocale(locale, "全局请求头", "Global headers")}
          </FieldLabel>
          <Textarea
            className="min-h-[92px] font-mono text-sm"
            value={upstreamHeadersConfig.global}
            onChange={(event) => onGlobalHeadersChange(event.target.value)}
            placeholder={
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-xs text-muted-foreground">
              {titleForLocale(
                locale,
                "按模型名匹配附加请求头；仍可被渠道组合请求头覆盖。",
                "Match by model name; still overridden by combination headers.",
              )}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {upstreamHeadersConfig.rules.map((rule, ruleIndex) => (
              ...
              >
                ...
                <Field>
                  <FieldLabel>
                    {titleForLocale(locale, "规则请求头", "Rule headers")}
                  </FieldLabel>
                  <Textarea
                    className="min-h-[84px] font-mono text-sm"
                    value={rule.headers}
                    onChange={(event) =>
                      onUpdateRule(ruleIndex, { headers: event.target.value })
                    }
