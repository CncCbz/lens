...
export function ChannelsScreen() {
  ...
  const savingSiteRef = useRef(false);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (savingSiteRef.current) return;
    if (!validateSiteForm()) return;
    savingSiteRef.current = true;
    setSavingSite(true);
    try {
  ...
  function updateModelProtocols(
    ...
    setForm((current) => ({
      ...current,
      protocolConfigs: current.protocolConfigs.map((protocolConfig, index) => {
        if (index !== protocolConfigIndex) return protocolConfig;
        return {
          ...protocolConfig,
          models: protocolConfig.models.map((model, currentModelIndex) =>
  ...
  function canTestModel(protocolConfigIndex: number, modelIndex: number) {
    const protocolConfig = form.protocolConfigs[protocolConfigIndex];
    const model = protocolConfig?.models[modelIndex];
    if (!protocolConfig || !model) return false;
    return modelTestOptionByKey.has(
      protocolConfigModelKey(protocolConfigIndex, protocolConfig, model),
    );
  }
      ...
      const payload: SiteModelFetchPayload = {
        ...
        credentials: form.credentials
          .map((item, index) => ({
            ...
            enabled: item.enabled,
            cost_multiplier: Number.isFinite(item.cost_multiplier)
              ? Math.max(item.cost_multiplier, 0)
              : 1,
          }))
