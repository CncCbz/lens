...
export const MAX_CHANNEL_CONCURRENCY = 2_147_483_647;
...
export type ChannelSort =
  "requests-desc" | "name-asc" | "name-desc" | "models-desc" | "protocols-desc";
...
export function SiteFavicon({ name }: { name: string }) {
  const letter = (name.trim()[0] || "?").toUpperCase();
  return (
    <span
      aria-hidden="true"
      className="flex size-11 items-center justify-center rounded-xl border bg-background/80 text-sm font-semibold text-muted-foreground"
    >
      {letter}
