...
import {
  ...
  type FormState as ChannelFormState,
...
const ModelTestDialog = dynamic(async () => {
  const loaded = await import("./channels/model-dialogs");
...
export function GroupsScreen() {
  ...
  const [modelTestForm, setModelTestForm] = useState<ChannelFormState | null>(
    null,
  );
    ...
    () =>
      (groups ?? []).map((group) => {
        ...
        return {
          ...
          disabled_channel_member_count: isRouteGroup
            ? 0
            : displayMembers.filter((member) => member.has_disabled_channel)
                .length,
  ...
  const previewGroups = useMemo(
    () => groupRows.filter((group) => group.protocols.length > 0),
    [groupRows],
  );
  const selectedPreviewGroup = useMemo(
    () =>
      previewGroups.find((group) => group.name === previewGroupName) ??
      previewGroups[0] ??
      null,
    [previewGroupName, previewGroups],
  );
  const {
    data: routePreview,
    error: routePreviewError,
    isFetching: routePreviewIsFetching,
    refetch: refetchRoutePreview,
  } = useQuery({
    queryKey: [
      "route-preview",
      previewProtocol,
      selectedPreviewGroup?.name ?? "",
    ],
    queryFn: () =>
      apiRequest<RoutePreviewResponse>("/admin/routes/preview", {
        method: "POST",
        body: JSON.stringify({
          protocol: previewProtocol,
          model: selectedPreviewGroup?.name ?? "",
        }),
      }),
    enabled: Boolean(
      selectedPreviewGroup &&
      selectedPreviewGroup.protocols.includes(previewProtocol),
    ),
    staleTime: 5_000,
  });
  ...
  useEffect(() => {
    ...
    if (previewGroupName !== selectedPreviewGroup.name) {
      setPreviewGroupName(selectedPreviewGroup.name);
      return;
    }
    if (!selectedPreviewGroup.protocols.includes(previewProtocol)) {
      setPreviewProtocol(selectedPreviewGroup.protocols[0]);
    }
  }, [previewGroupName, previewProtocol, selectedPreviewGroup]);
  ...
  function buildMemberModelTestForm(
    member: FoldedMember,
  ): ChannelFormState | null {
    const sourceItem =
      member.subItems.find((item) => item.enabled && item.protocol) ??
      member.subItems.find((item) => item.protocol) ??
      member.subItems[0];
    if (!sourceItem) return null;

    const protocolConfigId = protocolConfigIdFromChannelId(
      sourceItem.channel_id,
    );
    for (const site of sites ?? []) {
      const protocolConfig = site.protocols.find(
        (item) => item.id === protocolConfigId,
      );
      if (!protocolConfig) continue;

      const credential = site.credentials.find(
        (item) => item.id === sourceItem.credential_id,
      );
      const baseUrl =
        site.base_urls.find((item) => item.id === protocolConfig.base_url_id) ??
        site.base_urls.find((item) => item.enabled && item.url.trim()) ??
        site.base_urls[0];
      const protocols = member.protocols.length
        ? member.protocols
        : sourceItem.protocol
          ? [sourceItem.protocol]
          : [];
      if (!credential || !baseUrl || !protocols.length) return null;

      return {
        name: site.name,
        base_urls: site.base_urls.map((item) => ({ ...item })),
        ...
        protocolConfigs: [
          {
            ...
            enabled: protocolConfig.enabled,
            headers: Object.entries(protocolConfig.headers).map(
              ([key, value]) => ({ key, value }),
            ),
            ...
            models: [
              {
                ...
                protocolIds: {},
                credential_id: credential.id,
                model_name: sourceItem.model_name,
  ...
  function openMemberModelTest(member: FoldedMember) {
    const nextForm = buildMemberModelTestForm(member);
    const model = nextForm?.protocolConfigs[0]?.models[0];
    const protocols = modelSupportedProtocols(model);
    if (!nextForm || !protocols.length) {
      toast.error(
        locale === "zh-CN"
          ? "测试参数不完整"
    ...
    setModelTestForm(nextForm);
    setModelTestTarget({ protocolConfigIndex: 0, modelIndex: 0 });
  ...
  function buildModelTestPayload(): SiteModelTestPayload | null {
    if (!modelTestForm || !modelTestTarget) return null;

    const protocolConfig =
      modelTestForm.protocolConfigs[modelTestTarget.protocolConfigIndex];
    const model = protocolConfig?.models[modelTestTarget.modelIndex];
    const credentialIndex = model
      ? modelTestForm.credentials.findIndex(
          (item) => item.id === model.credential_id,
        )
      : -1;
    const credential =
      credentialIndex >= 0 ? modelTestForm.credentials[credentialIndex] : null;
    const activeBaseUrl = protocolConfig
      ? activeBaseUrlValue(modelTestForm, protocolConfig).trim()
      : "";
    const prompt = modelTestPrompt.trim();
    if (!protocolConfig || !model || !credential || !activeBaseUrl || !prompt) {
      return null;
    }
    ...
    <>
      ...
      <section className="flex flex-col gap-4">
        <GroupsOverview
          ...
          selectedPreviewGroup={selectedPreviewGroup}
          previewGroupName={previewGroupName}
          setPreviewGroupName={setPreviewGroupName}
          previewProtocol={previewProtocol}
          setPreviewProtocol={setPreviewProtocol}
          routePreview={routePreview}
          routePreviewIsFetching={routePreviewIsFetching}
          routePreviewError={routePreviewError}
          refetchRoutePreview={refetchRoutePreview}
          openEdit={openEdit}
        ...
        {modelTestTarget && modelTestForm ? (
          <ModelTestDialog
            ...
            onPromptChange={setModelTestPrompt}
