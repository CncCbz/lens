...
export function SeriesChip({
	...
	return (
		<button
			...
			onClick={onClick}
			className={cn(
				"group flex min-w-[56px] snap-start items-center justify-center rounded-xl border bg-card px-2.5 py-2 text-center transition-all",
				selected
		...
		>
			...
			>
				{isAll ? (
					...
					<ModelAvatar name={sampleModel} size={20} />
				)}
			</span>
		</button>
	);
