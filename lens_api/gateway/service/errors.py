...
def _protocol_error_payload(
    *,
    ...
    metadata = _protocol_error_metadata(
        request_id=request_id, attempt_count=attempt_count, retryable=retryable
    )
...
def _protocol_error_metadata(
    *, request_id: str | None, attempt_count: int | None, retryable: bool | None
) -> dict[str, Any]:
    metadata: dict[str, Any] = {}
    if request_id:
        metadata["request_id"] = request_id
    if attempt_count is not None:
        metadata["attempt_count"] = max(attempt_count, 0)
...
def _apply_router_runtime_settings(runtime: dict[str, Any]) -> None:
    app_state.router.configure_health_scoring(
        ...
        circuit_minimum_requests=int(runtime["router_circuit_minimum_requests"]),
        circuit_failure_rate_threshold=float(
            runtime["router_circuit_failure_rate_threshold"]
        ),
    )
...
async def dynamic_cors_middleware(
    ...
    try:
        ...
        return _apply_security_headers(_database_error_response(exc, request))
