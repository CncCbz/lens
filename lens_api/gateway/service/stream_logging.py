...
def _release_stream_concurrency(capture: StreamCapture | None) -> None:
    if capture is None or capture.concurrency_release is None:
        return
    release = capture.concurrency_release
    capture.concurrency_release = None
    release()


async def _close_stream_resources(capture: StreamCapture) -> None:
    response = capture.upstream_response
    try:
        if response is not None:
            await response.aclose()
    finally:
        capture.upstream_response = None
        client = capture.client_to_close
        capture.client_to_close = None
...
async def _record_stream_request_log(
    *,
    ...
    route_health_recorded = await _record_stream_route_health(
        channel=channel,
        capture=capture,
    ...
    if capture is not None:
        capture.route_health_recorded = route_health_recorded
    (
        input_cost_usd,
        output_cost_usd,
        total_cost_usd,
...
async def _record_stream_request_log_and_release_probe(
    *,
    target: RouteTarget,
    request_log_id: int,
    protocol: ProtocolKind,
    ...
    try:
        ...
        capture = result.stream_capture
        if capture is None or not capture.route_health_recorded:
...
async def _record_stream_route_health(
    *,
    ...
    model_name = _last_attempt_model_name(attempts)
    if capture_issue is None:
        app_state.router.record_success(
            channel.id,
            credential_id=credential_id,
            model_name=model_name,
            probe_owner=capture.probe_owner if capture is not None else None,
        )
        return True
    if _is_client_stream_disconnect(capture):
        # Release any probe lease without counting upstream failure.
        app_state.router.release_probe(
            RouteTarget(
                channel=channel,
                credential_id=credential_id,
                model_name=model_name,
...
def _last_attempt_model_name(attempts: list[dict[str, Any]]) -> str | None:
    if not attempts:
        return None
    model_name = attempts[-1].get("model_name")
