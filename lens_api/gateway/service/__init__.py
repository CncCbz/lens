from __future__ import annotations

import sys

from ...api import create_app
from ...core.config import settings
from ...models import (
    AdminProfile,
    AdminProfileUpdateResponse,
    AppInfo,
    AuthTokenResponse,
    ConfigImportResult,
    CronjobItem,
    CronjobRunResult,
    GatewayApiKey,
    ModelGroup,
    ModelGroupCandidatesResponse,
    ModelGroupEnsureFromSiteResponse,
    ModelPriceItem,
    ModelPriceListResponse,
    PiConfigExportResponse,
    PiConfigGenerateResponse,
    MultimodalRelayConfig,
    MultimodalRelayUpdate,
    OverviewDailyPoint,
    OverviewChannelAnalytics,
    OverviewChannelHealthPoint,
    OverviewDimensionUsageAnalytics,
    OverviewModelAnalytics,
    OverviewPerformanceAnalytics,
    OverviewSummary,
    PublicBranding,
    RequestLogDetail,
    RequestLogPage,
    RoutePreviewResponse,
    SettingItem,
    SiteBatchImportResult,
    SiteModelFetchItem,
    SiteModelTestResult,
    SiteRuntimeSummary,
)
from .admin_config import (
    create_gateway_api_key,
    create_model_group,
    delete_gateway_api_key,
    delete_model_group,
    ensure_model_groups_from_site,
    export_gateway_key_models_config,
    export_models_config,
    export_settings_bundle,
    generate_group_pi_config,
    get_model_group,
    get_multimodal_relay_config,
    import_settings_bundle,
    list_cronjobs,
    list_gateway_api_keys,
    list_model_groups,
    list_model_prices,
    list_settings,
    model_group_candidates,
    run_cronjob,
    sync_model_prices,
    update_cronjob,
    update_gateway_api_key,
    update_gateway_key_models_config,
    update_model_group,
    update_model_price,
    update_multimodal_relay_config,
    update_settings,
)
from .admin_sites import (
    clear_request_logs,
    clear_router_cooldown,
    create_site,
    delete_site,
    fetch_site_models,
    import_sites,
    list_sites,
    overview_channel_health,
    overview_daily,
    overview_models,
    overview_channels,
    overview_performance_channels,
    overview_performance_models,
    overview_summary,
    overview_usage_channels,
    overview_usage_gateway_keys,
    overview_usage_models,
    request_log_detail,
    request_log_page,
    route_preview,
    router_cooldowns,
    router_snapshot,
    site_runtime_summaries,
    test_site_model,
    update_site,
)
from .auth import (
    app_info,
    change_password,
    current_admin,
    login,
    public_branding,
    update_profile,
)
from .errors import (
    cors_preflight,
    dynamic_cors_middleware,
    register_exception_handlers,
)
from .lifecycle import lifespan
from .proxy_routes import (
    export_gateway_models_config,
    list_gateway_models,
    list_gemini_models,
    proxy_anthropic_messages,
    proxy_gemini_generate_content,
    proxy_gemini_stream_generate_content,
    proxy_openai_chat,
    proxy_openai_embeddings,
    proxy_openai_image_edits,
    proxy_openai_image_generations,
    proxy_openai_responses,
    proxy_rerank,
)

app = create_app(service_module=sys.modules[__name__])

__all__ = [
    "AdminProfile",
    "AdminProfileUpdateResponse",
    "AppInfo",
    "AuthTokenResponse",
    "ConfigImportResult",
    "CronjobItem",
    "CronjobRunResult",
    "GatewayApiKey",
    "ModelGroup",
    "ModelGroupCandidatesResponse",
    "ModelGroupEnsureFromSiteResponse",
    "ModelPriceItem",
    "ModelPriceListResponse",
    "PiConfigExportResponse",
    "PiConfigGenerateResponse",
    "OverviewDailyPoint",
    "OverviewChannelAnalytics",
    "OverviewChannelHealthPoint",
    "OverviewDimensionUsageAnalytics",
    "OverviewModelAnalytics",
    "OverviewPerformanceAnalytics",
    "OverviewSummary",
    "PublicBranding",
    "RequestLogDetail",
    "RequestLogItem",
    "RequestLogPage",
    "RoutePreviewResponse",
    "SettingItem",
    "SiteBatchImportResult",
    "SiteModelFetchItem",
    "SiteModelTestResult",
    "SiteRuntimeSummary",
    "app",
    "app_info",
    "change_password",
    "clear_request_logs",
    "clear_router_cooldown",
    "cors_preflight",
    "create_gateway_api_key",
    "create_model_group",
    "create_site",
    "current_admin",
    "delete_gateway_api_key",
    "delete_model_group",
    "delete_site",
    "dynamic_cors_middleware",
    "ensure_model_groups_from_site",
    "export_gateway_key_models_config",
    "export_settings_bundle",
    "fetch_site_models",
    "get_model_group",
    "get_multimodal_relay_config",
    "MultimodalRelayConfig",
    "MultimodalRelayUpdate",
    "import_settings_bundle",
    "import_sites",
    "lifespan",
    "list_cronjobs",
    "list_gateway_api_keys",
    "list_gateway_models",
    "list_gemini_models",
    "list_model_groups",
    "list_model_prices",
    "list_settings",
    "list_sites",
    "login",
    "model_group_candidates",
    "overview_channel_health",
    "overview_daily",
    "overview_models",
    "overview_channels",
    "overview_performance_channels",
    "overview_performance_models",
    "overview_summary",
    "overview_usage_channels",
    "overview_usage_gateway_keys",
    "overview_usage_models",
    "proxy_anthropic_messages",
    "proxy_gemini_generate_content",
    "proxy_gemini_stream_generate_content",
    "proxy_openai_chat",
    "proxy_openai_embeddings",
    "proxy_openai_responses",
    "proxy_rerank",
    "public_branding",
    "register_exception_handlers",
    "request_log_detail",
    "request_log_page",
    "route_preview",
    "router_cooldowns",
    "router_snapshot",
    "run_cronjob",
    "settings",
    "site_runtime_summaries",
    "sync_model_prices",
    "test_site_model",
    "update_cronjob",
    "update_gateway_api_key",
    "update_gateway_key_models_config",
    "update_model_group",
    "update_model_price",
    "update_multimodal_relay_config",
    "update_profile",
    "update_settings",
    "update_site",
]
