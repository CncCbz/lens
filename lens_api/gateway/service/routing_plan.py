...
from ..converters.openai_chat_compat import normalize_openai_chat_request
...
async def _resolve_routing_plan(
    ...
    matched_group = await app_state.group_repo.find_group_by_name(requested_model)
    if matched_group is None:
        raise LookupError(
            f"No model group named {requested_model} for protocol={protocol.value}"
        )
    if protocol not in matched_group.protocols:
        supported = ", ".join(item.value for item in matched_group.protocols) or "none"
        raise LookupError(
            f"Model group {matched_group.name} matched {requested_model} "
            f"but does not support protocol={protocol.value}; supported={supported}"
        )
    ...
    if matched_group.route_group_id.strip():
        ...
        if protocol not in resolved_group.protocols:
            supported = (
                ", ".join(item.value for item in resolved_group.protocols) or "none"
            )
            raise LookupError(
                f"Route target model group {resolved_group.name} for "
                f"{matched_group.name} does not support protocol={protocol.value}; "
    ...
    if not route_targets:
        raise LookupError(
            f"Model group {resolved_group.name} has no enabled route targets "
            f"for protocol={protocol.value}; requested={requested_model}"
        )
    return RoutingPlan(
...
def _prepare_upstream_body(
    ...
    if protocol == ProtocolKind.OPENAI_CHAT:
        payload = normalize_openai_chat_request(payload)
    elif protocol == ProtocolKind.OPENAI_RESPONSES and "input" in payload:
