...
from ...core.runtime_channel_ids import format_runtime_channel_name
from ..shared import (
    AsyncSession,
    ChannelStatus,
    ModelGroup,
...
class GroupRepository:
    ...
    async def find_group_by_name(self, name: str | None) -> ModelGroup | None:
    ...
    async def list_group_candidates(
        ...
        for model_key, aggregate in candidate_aggregates.items():
            ...
            matched_protocols: list[ProtocolKind] = []
            if protocols_filter:
                matched_protocols = [
                    requested_protocol
                    for requested_protocol in protocols_filter
                    if any(
                        can_reach_protocol(native_protocol, requested_protocol)
            ...
            if matched_protocols:
                for requested_protocol in matched_protocols:
                    if requested_protocol in aggregate.protocol_channels:
                        rep_protocol = requested_protocol
                        break
            ...
            if matched_protocols:
                ...
                for requested_protocol in matched_protocols:
                    if requested_protocol in aggregate.protocol_channels:
                        channel_id = aggregate.protocol_channels[requested_protocol]
                        ...
                        uncovered.append(requested_protocol)
                for requested_protocol in uncovered:
                    fallback_native = next(
                        (
                            native_protocol
                            for native_protocol in aggregate.native_protocols
                            if can_reach_protocol(native_protocol, requested_protocol)
                        ),
                        None,
                    )
    ...
    async def _load_group_item_channel_lookups(
        ...
        site_ids = {
            str(site_id)
            for _protocol_config_id, _combo_name, site_id, _site_name in site_rows
        }
        combo_count_by_site: dict[str, int] = {}
        combo_index_by_protocol_config: dict[str, int] = {}
        if site_ids:
            site_combo_rows = (
                await session.execute(
                    select(
            ...
            for protocol_config_id, site_id in site_combo_rows:
                site_id = str(site_id)
                combo_index_by_protocol_config[str(protocol_config_id)] = (
                    combo_counts.get(site_id, 0)
                )
                combo_counts[site_id] = combo_counts.get(site_id, 0) + 1
            combo_count_by_site = combo_counts
        channel_names_by_protocol_config: dict[str, str] = {}
        for protocol_config_id, combo_name, site_id, site_name in site_rows:
            protocol_config_id = str(protocol_config_id)
            site_id = str(site_id)
            channel_names_by_protocol_config[protocol_config_id] = (
                format_runtime_channel_name(
                    str(site_name),
                    str(combo_name or ""),
                    multi_combo=combo_count_by_site.get(site_id, 0) > 1,
                    combo_index=combo_index_by_protocol_config.get(protocol_config_id),
                )
            )
        ...
        for (
            ...
            channel_name = channel_names_by_protocol_config.get(protocol_config_id, "")
            ...
            for channel_id in channel_ids_for_config:
                channel_site_names[channel_id] = channel_name
