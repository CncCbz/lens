...
class ChannelUpsertsMixin:
    ...
    async def _upsert_base_urls(
        ...
        stale_ids = [row_id for row_id in existing if row_id not in next_ids]
        if stale_ids:
            await session.execute(
                delete(SiteBaseUrlEntity).where(SiteBaseUrlEntity.id.in_(stale_ids))
            )
        for index, item in enumerate(items):
            protocols_json = _dump_protocols_json(item.supported_protocols)
            ...
            if entity is None:
                session.add(
                    SiteBaseUrlEntity(
                        ...
                        sort_order=index,
                        supported_protocols_json=protocols_json,
                    )
                )
                continue
            entity.site_id = site_id
            entity.url = str(item.url)
            entity.name = item.name
            entity.enabled = int(item.enabled)
            entity.sort_order = index
            entity.supported_protocols_json = protocols_json
        ...
        if stale_ids:
            await session.execute(
                delete(SiteCredentialEntity).where(
                    SiteCredentialEntity.id.in_(stale_ids)
                )
            )
        for index, item in enumerate(items):
            entity = existing.get(item.id)
            ...
            entity.site_id = site_id
            entity.name = item.name
            entity.api_key = item.api_key
            entity.enabled = int(item.enabled)
            entity.cost_multiplier = float(item.cost_multiplier)
            entity.sort_order = index
    ...
    async def _upsert_protocol_configs(
        ...
        for protocol_config in protocol_configs:
            ...
            entity = existing_entities.get(protocol_config_id)
            if entity is None:
                entity = SiteProtocolConfigEntity(id=protocol_config_id)
                session.add(entity)
                existing_entities[protocol_config_id] = entity
            entity.site_id = site_id
            ...
            await self._upsert_protocol_config_models(
                ...
                protocol_config,
                credential_ids,
                existing_models_by_config.get(protocol_config_id, {}),
            )
            if (
    ...
    async def _upsert_protocol_config_models(
        ...
        credential_ids: set[str],
        existing_models: dict[str, SiteDiscoveredModelEntity] | None = None,
    ) -> None:
        ...
        for model_index, model in enumerate(
            ...
            next_ids.add(model_id)

            entity = existing.get(model_id)
            if entity is None:
                session.add(
                    SiteDiscoveredModelEntity(
                        id=model_id,
                ...
                continue
            entity.protocol_config_id = protocol_config_id
            entity.credential_id = model.credential_id
            entity.model_name = model_name
            entity.enabled = int(model.enabled)
            entity.sort_order = model_index
            entity.protocol = protocol_value
