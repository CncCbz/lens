...
class ChannelNormalizationMixin:
    def _flatten_site(self, site: SiteConfig) -> list[ChannelConfig]:
        ...
        multi_combo = len(site.protocols) > 1
        items: list[ChannelConfig] = []
        for combo_index, protocol_config in enumerate(site.protocols):
            ...
            channel_name = format_runtime_channel_name(
                site.name,
    ...
    def _build_channel_keys(
        ...
        return [
            ChannelKeyItem(
                ...
                cost_multiplier=credential.cost_multiplier,
            )
        ...
        for row in rows:
            result[row.site_id].append(
                SiteProtocolConfig(
                    ...
                    spent_cost_usd=row.spent_cost_usd,
