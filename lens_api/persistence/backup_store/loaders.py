...
class BackupLoadersMixin:
    ...
    async def _load_stats(self, session: AsyncSession) -> ConfigBackupStatsSnapshot:
        ...
        channel_daily_rows = (
            (
                await session.execute(
                    select(OverviewChannelDailyStatsEntity).order_by(
                        OverviewChannelDailyStatsEntity.date.asc(),
                        OverviewChannelDailyStatsEntity.channel_id.asc(),
                    )
                )
            )
            .scalars()
        ...
        dimension_daily_rows = (
            (
                await session.execute(
                    select(OverviewDimensionDailyStatsEntity).order_by(
                        OverviewDimensionDailyStatsEntity.date.asc(),
                        OverviewDimensionDailyStatsEntity.dimension_type.asc(),
        ...
        return ConfigBackupStatsSnapshot(
            ...
            dimension_daily=[
                ConfigBackupOverviewDimensionDailyStat(
                    ...
                    first_token_latency_ms_sum=row.first_token_latency_ms_sum,
                    input_tokens=row.input_tokens,
                    cache_read_input_tokens=row.cache_read_input_tokens,
                    cache_write_input_tokens=row.cache_write_input_tokens,
                    output_tokens=row.output_tokens,
                ...
                for row in dimension_daily_rows
            ],
        )
    ...
    async def _load_request_logs(
        ...
        for row in rows:
            ...
            logs.append(
                ConfigBackupRequestLog(
                    request_id=row.request_id or f"legacy-{row.id}",
