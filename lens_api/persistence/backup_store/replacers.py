...
class BackupReplacersMixin:
            ...
            for credential in site.credentials:
                ...
                session.add(
                    SiteCredentialEntity(
                        ...
                        cost_multiplier=float(credential.cost_multiplier),
            ...
            for protocol_config in site.protocols:
                ...
                session.add(
                    SiteProtocolConfigEntity(
                        ...
                        rpm_limit=protocol_config.rpm_limit,
                        token_limit=protocol_config.token_limit,
                        cost_limit_usd=protocol_config.cost_limit_usd,
                        spent_tokens=protocol_config.spent_tokens,
                        spent_cost_usd=protocol_config.spent_cost_usd,
    ...
    async def _replace_stats(
        ...
        channel_daily_keys: set[tuple[str, str]] = set()
        for item in stats.channel_daily:
            key = (item.date, item.channel_id)
            if key in channel_daily_keys:
                raise ValueError(
                    f"Duplicate channel stats row in backup: {item.date} {item.channel_id}"
                )
            channel_daily_keys.add(key)
            session.add(
                OverviewChannelDailyStatsEntity(
                    date=item.date,
        ...
        dimension_daily_keys: set[tuple[str, str, str]] = set()
        for item in stats.dimension_daily:
            key = (item.date, item.dimension_type, item.dimension_id)
            if key in dimension_daily_keys:
                raise ValueError(
                    "Duplicate dimension stats row in backup: "
                    f"{item.date} {item.dimension_type} {item.dimension_id}"
                )
            dimension_daily_keys.add(key)
            session.add(
                OverviewDimensionDailyStatsEntity(
                    date=item.date,
                    ...
                    successful_requests=item.successful_requests,
                    failed_requests=item.failed_requests,
                    latency_ms_sum=item.latency_ms_sum,
                    first_token_latency_ms_sum=item.first_token_latency_ms_sum,
                    input_tokens=item.input_tokens,
                    cache_read_input_tokens=item.cache_read_input_tokens,
                    cache_write_input_tokens=item.cache_write_input_tokens,
                    output_tokens=item.output_tokens,
